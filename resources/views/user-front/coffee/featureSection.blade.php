...
<section class="category-area category-3 ptb-100 bg-primary">
    ...
    @if ($userBs->features_section_top_shape_image)
        <div class="bg-shape">
            <img class="lazyload"
                data-src="{{ Uploader::getImageUrl(Constant::WEBSITE_IMAGE, $userBs->features_section_top_shape_image, $userBs) }}"
                alt="Bg Shape">
        </div>
    @endif
    @if ($userBs->features_section_bottom_shape_image)
        <div class="bg-shape h-auto">
            <img class="lazyload"
                data-src="{{ Uploader::getImageUrl(Constant::WEBSITE_IMAGE, $userBs->features_section_bottom_shape_image, $userBs) }}"
                alt="Bg Shape">
        </div>
    @endif
</section>
