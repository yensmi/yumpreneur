                    ...
                    <tbody>
                        @foreach ($coupons as $coupon)
                            <tr>
                                <td>{{$coupon->name}}</td>
                                <td>{{$coupon->code}}</td>
                                <td>{{($coupon->type == 'fixed' && $userBe->base_currency_symbol_position == 'left') ? $userBe->base_currency_symbol : ''}}{{$coupon->value}}{{$coupon->type == 'percentage' ? '%' : ''}}{{($coupon->type == 'fixed' && $userBe->base_currency_symbol_position == 'right') ? $userBe->base_currency_symbol : ''}}</td>
                                <td>
                                    @php
                                ...
                                <td>
                                    <a href="{{route('user.coupon.edit', $coupon->id)}}" class="btn btn-warning my-2 btn-sm">Edit</a>
                                    <form class="d-inline-block deleteform" action="{{route('user.coupon.delete')}}" method="POST">
                                        @csrf
        ...
        <div class="modal-body">
          ...
          <form id="ajaxForm" class="modal-form" action="{{route('user.coupon.store')}}" method="POST">
            ...
            <div class="row no-gutters">
                <div class="col-lg-6">
                    <div class="form-group">
                        <label for="">Minimum Spend ({{$userBe->base_currency_text}})</label>
